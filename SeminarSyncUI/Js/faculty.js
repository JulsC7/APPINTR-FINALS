...
async function loadDepartments() {
  try {
    ...
    document.getElementById("facDept").innerHTML = allDepartments
...
function renderFaculty(data) {
  if (!data.length) {
    facultyTableBody.innerHTML =
      '<tr><td colspan="5" class="px-6 py-10 text-center text-sm text-slate-500">No records found.</td></tr>';
    return;
  }
  facultyTableBody.innerHTML = data
    .map((f) => {
...
facultyForm.onsubmit = async (e) => {
  ...
  formData.append(
    "employee_number",
    document.getElementById("facEmpNum").value,
  );
  ...
  const url = editFacId
    ? `${API_BASE_URL}/faculty/${editFacId}/`
